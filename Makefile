...
kernel:
	LOG_LEVEL=$(LOG_LEVEL) cargo build \
		...
		-Z json-target-spec -p kernel \
		--release \
		$(if $(KERNEL_FEATURES),--features $(KERNEL_FEATURES),) \
		--target kernel/x86_64-kernel.json
	mkdir -p esp
	cp target/x86_64-kernel/release/kernel esp/kernel-x86_64
...
lldb-qemu:
	...
	lldb \
	  -o "target create ./target/x86_64-kernel/release/kernel" \
